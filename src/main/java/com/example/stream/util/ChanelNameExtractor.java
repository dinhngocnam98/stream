...
public class ChanelNameExtractor {
    ...
    private static final String GENERAL_REGEX_CHANNEL_ID = "channel-id=\"([^\"]*)\".*?,\\s*([^\\n]+)";
    ...
    public String extractId(String content) {
        Pattern pattern = Pattern.compile(GENERAL_REGEX_CHANNEL_ID);
        Matcher matcher = pattern.matcher(content);

        if (matcher.find()) {
            String match = matcher.group(1).trim().replaceAll("PlutoTV-", "");
    ...
    private String clean(String match) {
        ...
        } else if (match.contains("channel-id")) {

            match = match.replaceAll("channel-id=\"", "").replaceAll("\"", "").replaceAll("PlutoTV-", "");
